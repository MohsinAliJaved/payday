...
class AttendanceService extends TenantService
{
    ...
    {
        return $this->getAttr('now') ? $this->carbon($this->getAttr('now'))->parse() : now();
    }
    ...
    {
        return $this->getAttr('today') ? $this->carbon($this->getAttr('today'))->parse() : today();
    }
    ...
    public function validatePunchInDate($date, $compareTo = null): AttendanceService
    {
        ...
        throw_if(
            Carbon::parse(Carbon::parse($date)->toDateString())
                ->diffInDays($compareTo ? $compareTo : today()) > 1,
