...
class LeaveAssignService extends TenantService
{
    ...
    public function getWorkingShiftTime()
    {
        $workingShift = $this->getWorkingShift();

        return $this->memoize('working-shift-details' . $this->model->id, function () use ($workingShift) {
            $dayName = $this->carbon($this->getStartDate())->toDayInLowerCase();
